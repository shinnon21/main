...
(function () {
  ...
  if (net) {
    ...
    /* 初期ポイントの比率座標。横長=キャッチコピー(左・中央帯)と本人写真(右)を避け、
       コピー上部の余白に折れ線ネットワークを描く（先頭4点）＋左下隅に1点。
       縦長=コピー下の余白 */
    var SEED_DOTS = [
      { fx: .07, fy: .12 }, { fx: .22, fy: .07 }, { fx: .40, fy: .11 },
      { fx: .30, fy: .17 }, { fx: .085, fy: .90 }
    ];
    /* 縦長(モバイル)=コピーが上・写真が下。テキスト帯(上半分)は避け、写真の脇の
       余白(左ガター)に小さなネットワークを置く。先頭4点は左ガターでまとまり、
       折れ線がテキストや写真本体を横切らないようにする */
    var SEED_DOTS_PORTRAIT = [
      { fx: .06, fy: .60 }, { fx: .04, fy: .73 }, { fx: .08, fy: .86 },
      { fx: .05, fy: .95 }, { fx: .95, fy: .78 }
    ];
