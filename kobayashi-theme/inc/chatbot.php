...
function kb_chatbot_generate( $message, $history, $lang ) {
	...
	$system  = implode( "\n", array(
		'あなたは「小林慎之助 公式ホームページ」（' . home_url( '/' ) . '）の案内アシスタントです。以下のルールを必ず守ってください。',
		'1. 回答は、このプロンプト末尾の【公式サイト掲載情報】に書かれている内容だけを根拠にすること。',
		'2. 掲載情報にない事柄は、推測・想像・一般知識による補完（創作）を一切せず、「公開されている情報の中にはございません」と正直に伝えたうえで、お問い合わせフォーム（' . $contact . '）を案内すること。',
		'3. 数値・日付・固有名詞は掲載情報の表記をそのまま使うこと。言い換え・概算・四捨五入をしないこと。',
		'4. 小林慎之助本人になりすまさないこと。あくまで案内役として三人称で答え、本人の意見や感想を代弁しないこと。',
		'5. 小林慎之助・Weeave株式会社・このサイトの内容と関係のない依頼（一般的な調べ物、文章やコードの作成、翻訳、創作など）には応じず、このサイトに関するご質問をお願いすること。',
		'6. 回答は簡潔に（目安300字以内）。関連ページがあれば、そのURLを1つだけ添えてよい（掲載情報にあるURLのみ）。',
		'7. ' . ( 'en' === $lang ? 'Respond in English.' : '日本語で回答すること。' ),
		'',
...
add_action( 'wp_footer', function () {
	...
	$suggests = array(
		kb_t( 'どんな実績がありますか？', 'What kind of work has he done?' ),
		kb_t( '経歴を教えてください', 'Tell me about his career.' ),
		kb_t( '仕事の相談はどこからできますか？', 'How can I contact him for work?' ),
	);
	?>
	<div class="kb-chat" id="kbChat">
		<button type="button" class="kb-chat-fab" id="kbChatFab" aria-expanded="false" aria-controls="kbChatPanel" aria-label="<?php echo esc_attr( kb_t( 'AIアシスタントを開く', 'Open AI assistant' ) ); ?>">
			<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>
			<span class="kb-chat-fab-badge">AI</span>
		</button>
		<section class="kb-chat-panel" id="kbChatPanel" role="dialog" aria-label="<?php echo esc_attr( kb_t( 'AIアシスタント', 'AI assistant' ) ); ?>" hidden>
			<header class="kb-chat-head">
				<span class="kb-chat-face"><?php if ( $face ) : ?><img src="<?php echo esc_url( $face ); ?>" alt="" width="34" height="34"><?php else : ?><span class="init">SK</span><?php endif; ?></span>
				<span class="kb-chat-title"><?php echo esc_html( kb_t( '小林慎之助 AIアシスタント', 'Shinnosuke Kobayashi AI Assistant' ) ); ?><small>beta</small></span>
			...
			<p class="kb-chat-note"><?php echo esc_html( kb_t( '回答はAIがサイト掲載情報から自動生成します。掲載のない内容にはお答えできません。', 'Answers are generated by AI from the information published on this site. Questions beyond it cannot be answered.' ) ); ?></p>
			<div class="kb-chat-body" id="kbChatBody" aria-live="polite">
				<div class="kb-chat-msg model"><?php echo esc_html( kb_t( 'こんにちは！小林慎之助の公式サイトAIアシスタントです。実績・経歴・お知らせなど、サイトに掲載されている内容についてお答えします。', "Hi! I'm the AI assistant of Shinnosuke Kobayashi's official website. Ask me about his works, career, and news published on this site." ) ); ?></div>
